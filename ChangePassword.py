...
def main(root, token, email_id, table_name):
    ...
    NewPasswordLabel["text"] = "Old Password"
    ...
    ReEnterPasswordLabel["text"] = "New Password"
    ReEnterPasswordLabel.place(x=120,y=340,width=167,height=46)

    EnterOldPassword=tk.Entry(root, show = '*')
    EnterOldPassword["bg"] = "#000000"
    EnterOldPassword["borderwidth"] = "2px"
    ft = tkFont.Font(family='Times New Roman',size=14)
    EnterOldPassword["font"] = ft
    EnterOldPassword["fg"] = "#ffffff"
    EnterOldPassword["justify"] = "center"
    EnterOldPassword["text"] = ""
    EnterOldPassword.place(x=340,y=240,width=299,height=41)
    ...
    Done["command"] = lambda : Done_command(root, ConfirmPassword.get(), EnterOldPassword.get(), token, email_id, table_name)
    ...
    Back["justify"] = "center"
    Back["text"] = "Back"
    Back.place(x=10,y=10,width=156,height=40)
    Back["command"] = lambda : Back_command(root, token, email_id)
    hover(Back)
    ...
    C1["command"] = lambda : SHpassword(CheckVar1,EnterOldPassword)
...
def Done_command(root, C, N, token, email_id, table_name):
    ...
    print(email_id)
    verify_password_query = f"select password from {table_name} where email_id = '{email_id}'"
    Old_password = execute_query(verify_password_query, connection).fetchall()
    print(Old_password, N)

    if Old_password[0][0] == N:
...
def Back_command(root, token, username):
    if token == 'A':
